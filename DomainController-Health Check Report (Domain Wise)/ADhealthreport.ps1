...
$reportpath = "E:\HC Script\ADReport" + $date + ".htm"



if((test-path $reportpath) -like $true)
{
new-item $reportpath -type file
}
$smtphost = "SMTPAddress"
$from = "healthcheckreport@xx.com" 
$email1 = "Sam.Fisher@xx.com"
$timeout = "200"
...
add-content $report  "<font face='tahoma' color='#003399' size='4'><strong>Active Directory Health Check NtcgtecH</strong></font>" 
add-content $report  "</td>" 
...
Add-Content $report  "<td width='5%' align='center'><B>Site</B></td>" 
...
#$DCServers = $getforest.domains | Where-Object {$_.name -eq "ntcgtech.com"} | ForEach-Object {$_.DomainControllers} | ForEach-Object {$_.Name} 

#$DCServers = $get | ForEach-Object {$_.DomainControllers} | ForEach-Object {$_.Name} 

$DCservers = Get-ADDomainController -Discover -DomainName  "ntcgtech.com"  | % { Get-ADDomainController -server $_.Name -filter * } 

$DCservers_Sort = $DCservers | Sort-Object 


################Ping Test######

foreach ($DC in $DCServers_Sort){
$Identity = $DC
$Site = $DC.Site
                Add-Content $report "<tr>"
if ( (Test-Connection -ComputerName $DC -Count 1 -ErrorAction SilentlyContinue) -or ($DC -like "ntcgtechdc02") -or ($DC -like "ntcgtechdc04")) {
Write-Host $DC `t $DC `t Ping Success -ForegroundColor Green
        ...
        Add-Content $report "<td bgcolor= 'GainsBoro' align=center>  <B> $Site</B></td>" 
...
$subject = "Active Directory Health Monitor NtcgtecH" 
